...
import { byTestId } from "../support";

describe('Linqua', function () {
  before(() => {
    localStorage.clear();
    cy.clearIndexedDBs();
  ...
  it('empty list message is present', () => {
    cy.get(byTestId('empty-list-main-message')).should('contain', 'It is lonely here...');
  });

  it('add - translate - delete', () => {
    cy.get(byTestId('add-button')).click();
    cy.get(byTestId('entry-form')).within(() => {
      cy.get(byTestId('original-text-input')).type('voertuig');
      cy.get(byTestId('translate-button')).click();
      cy.get(byTestId('translation-text-input'), { timeout: 10000 }).should('have.value', "vehicle");
      cy.get(byTestId('submit-button')).click();
    });

    cy.get(byTestId('entry-list')).within(() => {
      cy.get(byTestId('original-text')).contains('voertuig').parents(byTestId('entry-list-item')).within(() => {
        cy.get(byTestId('open-action-menu-button')).click();
      });
    });

    cy.get(byTestId('delete-button')).click();
    cy.get(byTestId('empty-list-main-message')).should('contain', 'It is lonely here...');
  });
